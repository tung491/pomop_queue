...
def initialize_queue() -> Queue:
    queue = Queue()
    conn = sqlite3.connect(f"{str(Path.home())}/.task.db")
    cursor = conn.cursor()
...
def cli():
    ...
    add_parser = queue_subparse.add_parser('add', help='Add task into task queue')
    ...
    pomop_parser.add_argument('-B', '--nobrowser', help='Turn off browser-open notification',
                              action='store_false')
    pomop_parser.add_argument('target_id', help='Target ID of the pomodoro. Default the next '
    ...
    if not args.branchs:
        ...
        name = queue.pop() if not args.target_id else queue.pop_item(args.target_id)
